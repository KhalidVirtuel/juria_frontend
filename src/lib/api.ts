...
/** 401 -> logout (sauf sur les routes d'auth elles-mêmes) */
api.interceptors.response.use(
  (r) => r,
  (err) => {
    const url: string = err?.config?.url ?? "";
    const isAuthRoute = url.includes("/auth/login") || url.includes("/auth/register") || url.includes("/auth/waitlist");
...
export const authAPI = {
  ...
  waitlist: async (data: {
    ...
    const res = await api.post<WaitlistResponse>("/auth/waitlist", {
      first_name: data.firstName,
      last_name: data.lastName,
      email: data.email,
    });
...
/** Supprime une pièce jointe */
  ...
  addTimelineEntry: async (
    folderId: ID,
    data: { title: string; description?: string; type: string; date: string; showInCalendar?: boolean }

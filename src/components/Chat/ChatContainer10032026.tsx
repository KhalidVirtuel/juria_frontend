...
import { Plus, Send, Search, FileText, PenTool, Brain, MessageSquare, Save, Sparkles, LayoutGrid, ChevronRight, X, ShieldCheck, Gavel, AlertTriangle, Compass, Handshake, CheckSquare, Heart, Building2, Briefcase, Home, Car, Users, Clock, ClipboardList, Scale, ArrowLeft } from 'lucide-react';
...
interface ChatContainerProps {
  ...
  pendingUserMessage?: string | null; // ✅ Message utilisateur en attente (affiché avant streaming)
}

const ChatContainer: React.FC<ChatContainerProps> = ({ 
  messages, 
  pendingUserMessage, // ✅ NOUVEAU
  onSendMessage,
  inputValue,
  onInputChange,
  streamingSteps = [],
  ...
  // Dossier lié à la conversation active
  const currentFolder = (() => {
    const conv = conversations.find(c => c.id === activeConversationId);
    if (!conv?.folderId) return null;
    return folders.find(f => String(f.id) === String(conv.folderId)) ?? null;
  })();
...
const clearAllFiles = () => {
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex-shrink-0 bg-white border-b border-gray-100 shadow-sm">
          <div className="flex items-center justify-between px-6 h-14">
            <div className="flex items-center gap-3">
              <button
                onClick={() => navigate(-1)}
                className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors text-mezin"
              >
                <ArrowLeft className="w-5 h-5" />
              </button>
              <div>
                <p className="text-sm font-bold text-mezin leading-tight">{currentFolder.name}</p>
                {currentConversation?.title && (
                  <p className="text-xs text-gray-400 leading-tight">{currentConversation.title}</p>
                )}
              </div>
            </div>
            <div className="w-9 h-9 rounded-full bg-mezin flex items-center justify-center text-white text-sm font-semibold select-none">
              {userInitials}
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-6 apple-scrollbar pt-4 pb-32 px-32">
        ...
        {/* ✅ Message utilisateur en attente - affiché AVANT les steps de streaming */}
        {pendingUserMessage && (
          <ChatMessage
            message={{
